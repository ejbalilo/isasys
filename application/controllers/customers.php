...
class Customers extends Person_controller
{
	...
	function index()
	{
		...
		$data['canedit'] = ($this->Employee->has_permission('customers_edit',$this->Employee->get_logged_in_employee_info()->person_id))?true:false;
	...
	*/
	function view($customer_id=-1)
	{
		...
		{
			$customers[$row['person_id']] = $row['last_name'] .', '.$row['first_name'];
		}
	...
	*/
	...
	{
		if($customer_id==-1 || $this->Employee->has_permission('customers_edit',$this->Employee->get_logged_in_employee_info()->person_id))
		{
			$person_data = array(
			...
			'customer_discount'=>$this->input->post('customer_discount')=='' ? 0:$this->input->post('customer_discount'),
			'account_officer_id'=>$this->input->post('account_officer_id')=='' ? 0:$this->input->post('account_officer_id'),
			'sales_agent_id'=>$this->input->post('sales_agent_id')=='' ? 0:$this->input->post('sales_agent_id'),
			'sales_agent_commission'=>$this->input->post('sales_agent_commission')=='' ? 0:$this->input->post('sales_agent_commission'),
			'classification_id'=>$this->input->post('classification_id')=='' ? 0:$this->input->post('classification_id')
			);
	...
	*/
	function delete()
	{
		...
		if($this->Employee->has_permission('customers_edit',$this->Employee->get_logged_in_employee_info()->person_id))
		{
