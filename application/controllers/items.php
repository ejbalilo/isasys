...
class Items extends Secure_area implements iData_controller
{
	...
	*/
	...
	function view($item_id=-1)
	{
		...
		$data['item_prices']=$this->Item->get_prices($item_id);
	...
	function save($item_id=-1)
	{
		...
		'selling_name'=>trim($this->input->post('selling_name')),
		...
		'economic_quantity_level'=>$this->input->post('economic_quantity_level'),
		...
		if($this->Item->save($item_data,$item_id))
		{
			...
			$item_prices_data = array();
			$unit_prices = $this->input->post('unit_price');
			for($k=0;$k<count($unit_prices);$k++)
			{
				if(is_numeric($unit_prices[$k]))
				{
					$item_prices_data[] = array('unit_price'=>$unit_prices[$k],'sort_id'=>$k);
				}
			}
			$this->Item->save_prices($item_prices_data,$item_id);
		}
		else//failure
		{
	...
	*/
	...
	function transform($item_id=0)
	{
		$data = array();
		$this->load->view('items/transform',$data);
	}
}
?>