...
?>
	...
	<?php echo form_label('Customer Classification', 'customer_classification_label', array('class'=>'required')); ?>
	<div id='report_customer_classification'>
		<?php echo form_dropdown('classification',$classifications, 'all', 'id="classification"'); ?>
	</div>
	
	<div>
		Group by: <input type="radio" name="group_by" id="group_by_day" value='0' /> Day
		<input type="radio" name="group_by" id="group_by_month" value='1' checked='checked' /> Month
	</div>
...
$(document).ready(function()
{
	$("#generate_report").click(function()
	{
		...
		{
			window.location = window.location+'/'+$("#report_date_range_simple option:selected").val() + '/'+sale_type+'/'+group_by+'/'+export_excel + '/' + $("#classification").val();
		}
		else
		{
			...
			window.location = window.location+'/'+start_date + '/'+ end_date + '/'+sale_type+'/'+group_by+'/'+ export_excel + '/' + $("#classification").val();
		}
	});
