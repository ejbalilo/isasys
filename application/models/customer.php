...
{	
	...
	*/
	...
	function get_all_account_officers($limit=10000, $offset=0)
	{
		$this->db->select('DISTINCT(account_officer_id) AS person_id,last_name,first_name');
		$this->db->from('receivables');
		$this->db->join('people','receivables.account_officer_id=people.person_id');
	...
	function get_all_sales_agents($limit=10000, $offset=0)
	{
		$this->db->select('DISTINCT(sales_agent_id) AS person_id,last_name,first_name');
		$this->db->from('receivables');
		$this->db->join('people','receivables.sales_agent_id=people.person_id');
		$this->db->order_by("last_name,first_name", "asc");
		$this->db->limit($limit);
		$this->db->offset($offset);
		return $this->db->get();
	...
	{
		$this->db->from('customer_classifications');
		$this->db->where('deleted',0);
		return $this->db->get();
	}
}
