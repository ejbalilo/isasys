...
class Specific_account_officer extends Report
{
	...
	public function getDataColumns($show_profit=0)
	{
		...
		$summary_headers = array_merge($summary_headers,array("Balance", $this->lang->line('reports_comments')));
	...
	public function getData(array $inputs)
	{
		$query = $this->db->query("
			...
			WHERE status>0 AND DATE(receivable_date) BETWEEN '".$inputs['start_date']."' AND '".$inputs['end_date']."' AND account_officer_id = ".$inputs['account_officer_id']."
	...
	public function getSummaryData(array $inputs)
	{
		...
		$query = $this->db->query("
			...
			WHERE mode=0 AND DATE(receivable_date) BETWEEN '".$inputs['start_date']."' AND '".$inputs['end_date']."' AND ospos_receivables.account_officer_id = ".$inputs['account_officer_id']);
